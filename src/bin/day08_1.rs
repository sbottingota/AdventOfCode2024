...
fn main() {
    ...
    for antenna1 in &antennas {
        for antenna2 in &antennas {
            ...
            antinodes.insert((antenna1.pos.0 + dist.0, antenna1.pos.1 + dist.1));
            antinodes.insert((antenna2.pos.0 - dist.0, antenna2.pos.1 - dist.1));
        }
    }

    println!("{:?}", antinodes.into_iter().filter(|&(x, y)| x >= 0 && x < dims.0 && y >= 0 && y < dims.1).count());
}
